...
def load_nat_qx_data():
    ...
    try:
        ...
        try:
            # Fallback CSV name based on upload
    ...
    # Excel'den 28.0 olarak gelebilecek değerleri temizle
...
# --- E. PROVINCIAL MAP DATA (k values) ---
@st.cache_data
def load_prov_map_k():
...
# -----------------------------------------------------------------------------
# 6. TABS INTERFACE (MACRO TO MICRO)
# -----------------------------------------------------------------------------
...
with tab1:
    ...
    if not df_nat_qx.empty:
        ...
        st.error("National qx data not found.")
        ...
        with col2:
            ...
            if not filtered_map.empty:
                ...
                st.markdown(
                    ...
                    ℹ️ <b>Note:</b> Green areas indicate missing data for the selected year/sex.
                    </div>
...
# =============================================================================
# TAB 6: PROVINCIAL MAPS (67 Province Logic)
# =============================================================================
with tab6:
    ...
    if not df_prov_k.empty and turkey_geojson:
        ...
        with col2:
            ...
            if not raw_k_clean.empty:
                ...
                max_val = raw_k_clean.loc[max_idx, 'k']
                max_prov = raw_k_clean.loc[max_idx, 'province']
                sc2.metric(f"📈 Highest: {max_prov}", f"{max_val:.3f}")
                
                min_idx = raw_k_clean['k'].idxmin()
                min_val = raw_k_clean.loc[min_idx, 'k']
                min_prov = raw_k_clean.loc[min_idx, 'province']
